...
Introduction:
    ...
    Algorithm is tabular Q-learning
Author:
    chend
Modify:
    2020-03-30
...
class Tabular_Q_learning():
    ...
    def updata_q_table(self, state, action, reward, state_):
        '''q_learning method'''
        self.check_state_exist(state_)
        q_predict = self.q_table.loc[state, action]
        
        if state_ != 'terminal':
            q_target = reward + self.discount_factor * self.q_table.loc[state_, :].max()
        else:
...
def maze_simulation(max_episode, env, RL_algorithm):
    ...
    for episode in range(max_episode):
        ...
        while True:
            ...
            # RL learn from this transition <s,a,r,s'>
            RL_algorithm.updata_q_table(str(state), action, reward, str(state_))
...
def main():
    ...
    # sarsa
    RL_algorithm = Tabular_Q_learning(action_space, alpha=0.01, gamma=0.9, e_greedy=0.9)
    ...
    # plot total rewards
    plt.plot(total_reward, 'blue', lw=1)
    ...
    plt.savefig('MAZE_Qlearning_reward.png', dpi = 300)
    plt.close()
